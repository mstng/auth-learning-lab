...
Session画面で **「5分ガイドを始める」** を押してください。この学習空間のSessionと画面の実行記録を初期化し、次の4操作を案内します。ガイドは入力済みの一般ユーザーと5分の期限を使います。
...
5. http://localhost:3000 を再読み込みし、右上にv1.3、Session画面にガイドが表示されることを確認します。
...
| /session | 5分ガイド、ログイン9ステップ＋別操作のProfile4ステップ、前後比較、結果による達成確認 |
...
1. Session認証ページで「ログインする」を実行。9ステップの記録が表示され、Profileはまだ取得しません。
