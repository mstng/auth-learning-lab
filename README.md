...
- 4つの実験を終えたら、自分の言葉で説明するための問いと説明例を表示します。
...
5. http://localhost:3000 を再読み込みし、右上にv1.4、Session画面に「順番に学ぶ」が表示されることを確認します。
...
| /session | 既定の「順番に学ぶ」（4つの問い）と「自由に実験する」（全記録・データビューア） |
