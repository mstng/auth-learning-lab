...
test("focused learning shows one question, replays evidence and preserves experiments", async ({ page, context }) => {
  ...
  await expect(page.getByRole("button", { name: "ログインして確かめる", exact: true })).toBeDisabled();
  await page.getByRole("radio", { name: "ブラウザにパスワードが保存される", exact: true }).check();
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: "test-results/prediction-preview.png", fullPage: true });
  if (await page.locator(".prediction-choices").isVisible() && await page.locator(".prediction-choices input:checked").count() === 0) await page.getByRole("radio", { name: "まだわからない", exact: true }).check();
  ...
  await expect(page.locator('.number-chip[data-session-id="'+cookie.value+'"]')).toHaveCount(0);
  await page.getByRole("button", { name: "次へ", exact: true }).click();
  await expect(page.locator('[data-place="server"] .number-chip')).toHaveAttribute('data-session-id', cookie.value);
  await expect(page.locator('[data-place="browser"] .number-chip')).toHaveAttribute('data-session-id', '');
  await expect(page.locator('[data-place="database"] .number-chip')).toHaveAttribute('data-session-id', '');
  await page.getByRole("button", { name: "次へ", exact: true }).click();
  ...
  await expect(page.locator('[data-place="database"] .number-chip')).toHaveAttribute('data-session-id', cookie.value);
  await expect(page.locator('.travel-packet')).toHaveAttribute('data-session-id', cookie.value);
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await expect(page.locator('.travel-packet')).toHaveCSS('animation-name', 'none');
  ...
  await page.getByRole("radio", { name: "ログインで保存した受付番号", exact: true }).check();
  ...
  await expect(page.locator(".prediction-feedback")).toHaveAttribute("data-outcome", "match");
  await page.getByRole("button", { name: "次の問いへ", exact: true }).click();
  ...
  await page.getByRole("button", { name: "次の問いへ", exact: true }).click();
  await page.getByRole("radio", { name: "前にログインしたので見られる", exact: true }).check();
...
test("learning recovers when the session expires between questions", async ({ page }) => {
  await page.goto("/session");
  await page.getByRole("button", { name: "5分ガイドを始める", exact: true }).click();
  await expect(page.locator(".prediction-choices")).toBeVisible();
  if (await page.locator(".prediction-choices").isVisible() && await page.locator(".prediction-choices input:checked").count() === 0) await page.getByRole("radio", { name: "まだわからない", exact: true }).check();
  await page.getByRole("button", { name: "ログインして確かめる", exact: true }).click();
  await expect(page.locator(".tour-count")).toHaveText("1 / 4 確認できた");
  for (let i = 0; i < 5; i++) await page.getByRole("button", { name: "次へ", exact: true }).click();
  await page.getByRole("button", { name: "次の問いへ", exact: true }).click();
...
async function readFrames(page: import('@playwright/test').Page, count: number) {
  for (let i = 0; i < count; i++) await page.getByRole('button', { name: '次へ', exact: true }).click();
}
async function prepareExpiredLesson(page: import('@playwright/test').Page) {
  await page.goto('/session');
  await page.getByRole('button', { name: '失敗する実験から始める', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('0 / 5 確認できた');
  await page.getByRole('radio', { name: '番号は作られるが、情報は見られない', exact: true }).check();
  await page.getByRole('button', { name: '間違ったパスワードで試す', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('1 / 5 確認できた');
  ...
  await expect(page.locator('.tour-count')).toHaveText('2 / 5 確認できた');
  await readFrames(page, 4);
  await page.getByRole('button', { name: '次の問いへ', exact: true }).click();
  await page.getByRole('radio', { name: 'Cookieの番号も一緒に消える', exact: true }).check();
  await page.getByRole('button', { name: 'DBの期限だけ切らす', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('3 / 5 確認できた');
  await readFrames(page, 2);
  await expect(page.locator('.lesson-explanation')).toContainText('ブラウザには同じ番号が残っています');
}

test('failure guide traces the same expired number and replaces it after re-login', async ({ page, context }) => {
  const errors: string[] = [];
  page.on('pageerror', e => errors.push(e.message));
  await prepareExpiredLesson(page);
  const oldId = (await context.cookies()).find(c => c.name === 'lab_session')!.value;
  ...
  await expect(page.locator('.tour-count')).toHaveText('4 / 5 確認できた');
  await expect(page.locator('.travel-packet')).toHaveAttribute('data-session-id', oldId);
  await readFrames(page, 3);
  await expect(page.locator('.lesson-explanation')).toContainText('番号は送られましたが');
  await expect(page.locator('.lesson-explanation')).toContainText('取得拒否（401）');
  await expect(page.locator('.prediction-feedback')).toHaveAttribute('data-outcome', 'different');
  await expect(page.locator('[data-place="server"]')).toContainText('本人確認できず拒否');
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.evaluate(() => document.fonts.ready);
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.screenshot({ path: 'test-results/failure-preview.png' });
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await page.screenshot({ path: 'test-results/failure-mobile.png', fullPage: true });
  await page.getByRole('button', { name: '次の問いへ', exact: true }).click();
  await page.getByRole('radio', { name: 'まだわからない', exact: true }).check();
  await page.getByRole('button', { name: 'ログインし直して確かめる', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('5 / 5 確認できた');
  const newId = (await context.cookies()).find(c => c.name === 'lab_session')!.value;
  expect(newId).not.toBe(oldId);
  await expect(page.locator('[data-place="browser"] .number-chip')).toHaveAttribute('data-session-id', oldId);
  await expect(page.locator('[data-place="server"] .number-chip')).toHaveAttribute('data-session-id', newId);
  await readFrames(page, 4);
...
test('failure guide does not mistake a missing-Cookie 401 for an expired-Cookie 401', async ({ page, context }) => {
  await prepareExpiredLesson(page);
  await page.getByRole('button', { name: '次の問いへ', exact: true }).click();
  await context.clearCookies({ name: 'lab_session' });
  await page.getByRole('radio', { name: 'まだわからない', exact: true }).check();
  await page.getByRole('button', { name: '期限切れの番号でアクセス', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('3 / 5 確認できた');
  await expect(page.locator('.lesson-explanation')).toContainText('想定した結果を確認できませんでした');
  await expect(page.locator('.prediction-feedback')).toHaveCount(0);
  await expect(page.getByRole('button', { name: '次へ', exact: true })).toHaveCount(0);
  await page.getByRole('button', { name: '最初からやり直す', exact: true }).click();
  await expect(page.locator('.tour-count')).toHaveText('0 / 5 確認できた');
});
